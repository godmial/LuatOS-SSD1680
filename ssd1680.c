...
{
    // LUAT_DEBUG_PRINT("cmd %x", cmd);
    // LUAT_DEBUG_PRINT("cmd %s", data);
    // LUAT_DEBUG_PRINT("cmd %d", data_size);
...
static void ssd1680_setup_ram()
{
    ...
    ywnd_stop = 0;
    // switch (default_disp.orientation)
    // {
    // case SSD1680_90_DEG:
    //     b = 0x6;
    //     xwnd_start = default_disp.clmn_cnt - 1;
    //     xwnd_stop = 0;
    //     ywnd_start = 0;
    //     ywnd_stop = default_disp.rows_cnt - 1;
    //     break;
...
int ssd1680_init(void *spi_host, ssd1680_pinmap_t pinmap, uint16_t res_x, uint16_t res_y, ssd1680_orientation_t orientation)
{
    ...
    // default_disp.framebuffer_bw = default_disp.clmn_cnt * default_disp.rows_cnt;
    // if (default_disp.framebuffer_bw == NULL)
    //     goto error;
...
{
    // LUAT_DEBUG_PRINT("x :%d", x);
    // LUAT_DEBUG_PRINT("y :%d", y);
    // LUAT_DEBUG_PRINT("color :%d", color);
    ...
    //     idx = (x >> 3) + y * default_disp.clmn_cnt;
    //     offset = 7 - (x % 8);
    //     break;
    // }
...
void ssd1680_send_framebuffer()
{
    ...
    // {
    // case SSD1680_90_DEG:
    //     ssd1680_set_ram_pos(default_disp.clmn_cnt - 1, 0);
    //     break;
    // case SSD1680_180_DEG:
    //     ssd1680_set_ram_pos(default_disp.clmn_cnt - 1, default_disp.rows_cnt - 1);
    //     break;
    // case SSD1680_270_DEG:
    //     ssd1680_set_ram_pos(0, default_disp.rows_cnt - 1);
    //     break;
    // default: // SSD1680_NORMAL
    //     ssd1680_set_ram_pos(0, 0);
    //     break;
    // }
...
void ssd1680_set_refresh_window(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    ...
    // {
    // case SSD1680_90_DEG:
    //     ssd1680_set_x_window(y2 >> 3, y1 >> 3);
    //     ssd1680_set_y_window(x1, x2);
    //     break;
    // case SSD1680_180_DEG:
    //     ssd1680_set_x_window(x2 >> 3, x1 >> 3);
    //     ssd1680_set_y_window(y2, y1);
    //     break;
    // case SSD1680_270_DEG:
    //     ssd1680_set_x_window(y1 >> 3, y2 >> 3);
    //     ssd1680_set_y_window(x2, x1);
    //     break;
    // default: // SSD1680_NORMAL
    //     ssd1680_set_x_window(x1 >> 3, x2 >> 3);
    //     ssd1680_set_y_window(y1, y2);
    //     break;
    // }
}
...
void ssd1680_show_char(uint16_t x, uint16_t y, uint16_t chr, uint16_t size1, ssd1680_color_t color)
{
    ...
    // LUAT_DEBUG_PRINT("x :%d", x);
    // LUAT_DEBUG_PRINT("y :%d", y);
    // LUAT_DEBUG_PRINT("chr :%d", chr);
    // LUAT_DEBUG_PRINT("size :%d", size1);
    // LUAT_DEBUG_PRINT("color :%d", color);
 ...
 * @param color  颜色
 */
void ssd1680_nkd_show_char(uint16_t x, uint16_t y, uint16_t chr, uint16_t size1, ssd1680_color_t color)
{
    uint16_t i, m, temp, size2, chr1;
    uint16_t x0, y0;
    x += 1, y += 1, x0 = x, y0 = y;
    ...
    for (i = 0; i < size2; i++)
    {
        ...
        {
        case 16:
            temp = nkd_1608[chr1][i];
            break;

        case 20:
            temp = nkd_2010[chr1][i];
            break;

        case 36:
            temp = nkd_3618[chr1][i];
            break;

        case 48:
            temp = nkd_4824[chr1][i];
            break;
 ...
 * @param color 0,反色显示;1,正常显示
 */
void ssd1680_nkd_show_string(uint16_t x, uint16_t y, uint8_t *chr, uint16_t size1, ssd1680_color_t color)
{
    while (*chr != '\0') // 判断是不是非法字符!
    {
        ssd1680_nkd_show_char(x, y, *chr, size1, color);
        chr++;
        x += size1 / 2;
    }
}
 ...
 * @param color 字体颜色
 */
void ssd1680_nkd_show_number(uint16_t x, uint16_t y, uint32_t num, uint16_t size1, ssd1680_color_t color)
{
    uint8_t len = getNumberCount(num);
    uint8_t t, temp, m = 0;
    if (size1 == 8)
    ...
    for (t = 0; t < len; t++)
    {
        temp = (num / ssd1680_pow(10, len - t - 1)) % 10;
        if (temp == 0)
            ssd1680_nkd_show_char(x + (size1 / 2 + m) * t, y, '0', size1, color);
        else
            ssd1680_nkd_show_char(x + (size1 / 2 + m) * t, y, temp + '0', size1, color);
    }
}
...
void ssd1680_show_qrcode(uint8_t Xoffset, uint8_t Yoffset, char *input)
{
    // LUAT_DEBUG_PRINT("x :%d", Xoffset);
    // LUAT_DEBUG_PRINT("y :%d", Yoffset);
    // LUAT_DEBUG_PRINT("input :%s", input);
    // char inputqqqq[] = "https://www.vegeaiot.com?json={\"version\":\"V001\",\"deviceType\":\"hjf\",\"deviceId\":\"%jjhhj\",\"iccid\":\"hjhjh\",\"imei\":\"tstvr\"}";

    // ssd1680_fill(SSD1680_WHITE);
    QRGenerator(Xoffset, Yoffset, input);
    // ssd1680_send_framebuffer();
    // ssd1680_refresh(SSD1680_Global_Refresh);
}
