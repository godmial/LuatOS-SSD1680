...
static int l_ssd1680_nkd_show_string(lua_State *L)
{
    size_t len = 0;
    int x = luaL_checkinteger(L, 1);
    int y = luaL_checkinteger(L, 2);

    const char *chr = luaL_checklstring(L, 3, &len);
    int size1 = luaL_checkinteger(L, 4);
    ssd1680_color_t color = luaL_checkinteger(L, 5);

    ssd1680_nkd_show_string((uint16_t)x, (uint16_t)y, chr, (uint16_t)size1, color);
}

static int l_ssd1680_nkd_show_number(lua_State *L)
{
    size_t len = 0;
    int x = luaL_checkinteger(L, 1);
    int y = luaL_checkinteger(L, 2);
    ...
    ssd1680_nkd_show_number((uint16_t)x, (uint16_t)y, (uint32_t)num, (uint16_t)size1, color);
}
...
static const rotable_Reg_t reg_rink[] =
    {
        ...
        {"nkd_show_string", ROREG_FUNC(l_ssd1680_nkd_show_string)},
        {"nkd_show_number", ROREG_FUNC(l_ssd1680_nkd_show_number)},
        {NULL, ROREG_INT(0)},
